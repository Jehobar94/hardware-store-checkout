...
function Checkout({ cart, text, onClose }) {
  ...
  const pay = async () => {
    ...
    } catch (error) { setPaymentStatus('error'); setPaymentMessage(error instanceof TypeError ? 'No se pudo conectar con el servidor de pagos. Verifica que el backend esté iniciado.' : error.message); }
  };
